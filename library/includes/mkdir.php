...
if (isset($_POST['submit'])) {
    ...
    ++$lastinsert;
    $name = functions::checkin($_POST['name']);
    $desc = functions::checkin($_POST['description']);
    $type = intval($_POST['type']);
    $sql = "INSERT INTO `library_cats`
        (`parent`, `name`, `description`, `dir`, `pos`) 
    VALUES
        (" . $id . ", '" . mysql_real_escape_string($name) . "', '" . mysql_real_escape_string($desc) . "', " . $type . ", " . $lastinsert . ")";
    if (mysql_query($sql)) {
