    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($_FILES['textfile']['name'])) {
            ...
            if (mb_strtolower(end($ext)) == 'txt') {
                ...
                if (move_uploaded_file($_FILES['textfile']['tmp_name'], '../files/system/_tmp/' . $newname)) {
                    $txt = file_get_contents('../files/system/_tmp/' . $newname);
                    ...
                    $text = trim($txt);
                    unlink('../files/system/_tmp' . DIRECTORY_SEPARATOR . $newname);
                } else {
