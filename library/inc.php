...
#$kmess = 10; // the amount on the page
$i = 0;
...
class Tree
{
    ...
    /**
    * Recursively goes to the root of the tree, collecting an array of ids and names of sections
    * 
    * @param integer $id
    * @return Tree
    */
    ...
    /**
    * Recursively runs on wood collecting an array types and unique ides of catalogs
    * 
    * @param integer $id
    * @return Tree
    */
    ...
    /**
    * Cleaning articles removes comments, pictures and tags for articles
    * 
    * @param mixed $data
    * @return array
    */
    public function clean_trash($data) {
        if (!is_array($data)) {
            ...
            if (file_exists('../files/library/small/' . $data . '.png')) {
                unlink('../files/library/big/' . $data . '.png'); 
                unlink('../files/library/orig/' . $data . '.png');
                unlink('../files/library/small/' . $data . '.png');
    ...
    /**
    * Removes branch, returns the number of remote directories, articles, tags, comments, and images in an array
    * 
    * @param void
    * @return array
    */
    ...
    /**
    * Recursively goes through a branch and collects child attachments
    * 
    ...
    */
    ...
    /**
    * Collects links in the top navigation bar
    * 
    * @param void
    * @return string
    */ 
    public function print_nav_panel()
    {
        ...
        foreach ($array as $k => $v) {
            $return[] = $x == $cnt ? '<strong>' . htmlspecialchars($v) . '</strong>' : '<a href="?do=dir&amp;id=' . $k . '">' . functions::checkout($v) . '</a>';
            $x++;
        }
...
class Hashtags
{
    ...
    public function get_all_stat_tags($tpl = 0)
    {
        ...
        if ($cnt) {
            ...
            return null; // No articles have tags
        }
    }
    ...
    {
        $tag = preg_replace(array('/[^[:alnum:]]/ui', '/\s\s+/'), ' ', preg_quote(mb_strtolower($tag)));
        return trim($tag);
    }
    ...
    public function del_cache() {
        file_exists('../files/system/cache/cmpranglibcloud.dat') ? unlink('../files/system/cache/cmpranglibcloud.dat') : false;
        file_exists('../files/system/cache/cmpalphalibcloud.dat') ? unlink('../files/system/cache/cmpalphalibcloud.dat') : false;
    }
    
    public function get_cache($sort = 'cmpalpha') {
        if (file_exists('../files/system/cache/' . $sort . 'libcloud.dat')) {
            return file_get_contents('../files/system/cache/' . $sort . 'libcloud.dat');
        } else {
    ...
    public function set_cache($sort = 'cmpalpha') {
        ...
        file_put_contents('../files/system/cache/' . $sort . 'libcloud.dat', $res);
