...
if (preg_match("/[^1-9a-z\-\@\*\(\)\?\!\~\_\=\[\]]+/", mb_strtolower($search)))
...
if ($search && !$error) {
    ...
    */
    ...
    if ($total > $kmess)
        echo '<div class="topmenu">' . functions::display_pagination('search.php?search=' . urlencode($search) . '&page=', $start, $total, $kmess) . '</div>';
    if ($total > 0) {
        ...
        while ($res = mysql_fetch_assoc($req)) {
            ...
            $res['account'] = mb_strlen($search) < 2 ? $res['account'] : preg_replace('|('.preg_quote($search, '/').')|siu','<span style="background-color: #FFFF33">$1</span>', $res['account']);
