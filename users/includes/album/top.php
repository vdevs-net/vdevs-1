...
switch ($mod) {
    ...
    case 'votes':
        ...
        */
        ...
        $join = "";
        $where = "(`vote_plus` - `vote_minus`) > 2" . ($rights >= 6 ? "" : " AND `cms_album_files`.`access` = '4'");
        $order = "`rating` DESC";
        $link = '&amp;mod=votes';
        break;
    ...
    default:
        ...
        */
        ...
        $select = "";
        $join = "";
        $where = "`cms_album_files`.`time` > '" . (time() - 259200) . "'" . ($rights >= 6 ? "" : " AND `cms_album_files`.`access` = '4'");
...
if ($total) {
    if ($total > $kmess) {
        echo '<div class="topmenu">' . functions::display_pagination('album.php?act=top' . $link . '&page=', $start, $total, $kmess) . '</div>';
    }

    $req = mysql_query("
      SELECT `cms_album_files`.*, `users`.`account` AS `user_name`, `cms_album_cat`.`name` AS `album_name` $select
    ...
    for ($i = 0; ($res = mysql_fetch_assoc($req)) !== false; ++$i) {
        ...
        if ($res['access'] == 4 || $rights >= 7) {
            ...
            echo '<a href="album.php?act=show&amp;al=' . $res['album_id'] . '&amp;img=' . $res['id'] . '&amp;user=' . $res['user_id'] . '&amp;view"><img src="' . SITE_URL . '/files/users/album/' . $res['user_id'] . '/' . $res['tmb_name'] . '" /></a>';
            ...
            echo '<a href="album.php?act=show&al=' . $res['album_id'] . '&img=' . $res['id'] . '&amp;user=' . $res['user_id'] . '"><img src="' . SITE_URL . '/images/stop.gif" width="50" height="50"/></a>';
        }
