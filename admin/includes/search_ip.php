...
if ($search && !$error) {
    ...
    */
    ...
    if ($total > $kmess) {
        echo '<div class="topmenu">' . functions::display_pagination('index.php?act=search_ip' . ($mod == 'history' ? '&mod=history' : '') . '&search=' . urlencode($search) . '&page=', $start, $total, $kmess) . '</div>';
    }
    if ($total) {
        if ($mod == 'history') {
            $req = mysql_query("SELECT `cms_users_iphistory`.*, `users`.`account`, `users`.`rights`, `users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`, `users`.`id`, `users`.`browser`
