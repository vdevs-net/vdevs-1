    ...
    default:
        ...
        if (mysql_num_rows($req)) {
            while ($res = mysql_fetch_array($req)) {
                ...
                $arg = array (
                    'header' => '<br /><img src="' . SITE_URL . '/images/block.gif" width="16" height="16" align="middle" />&#160;<small><a href="' . SITE_URL . '/users/profile.php?act=ban&amp;user=' . $res['id'] . '">' . $lng_ban['infringements_history'] . '</a> [' . $res['bancount'] . ']</small>'
                );
        ...
        if ($total > $kmess) {
            echo '<div class="topmenu">' . functions::display_pagination('index.php?act=ban_panel&page=', $start, $total, $kmess) . '</div>';
