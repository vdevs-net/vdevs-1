...
while (($res = mysql_fetch_assoc($req)) !== false) {
    $link = '';
    if ($rights >= 7)
        $link .= '<a href="' . SITE_URL . '/users/profile.php?act=edit&amp;user=' . $res['id'] . '">' . $lng['edit'] . '</a> | <a href="index.php?act=usr_del&amp;id=' . $res['id'] . '">' . $lng['delete'] . '</a> | ';
    $link .= '<a href="' . SITE_URL . '/users/profile.php?act=ban&amp;mod=do&amp;user=' . $res['id'] . '">' . $lng['ban_do'] . '</a>';
...
if ($total > $kmess) {
    echo '<div class="topmenu">' . functions::display_pagination('index.php?act=usr&sort=' . $sort . '&page=', $start, $total, $kmess) . '</div>';
