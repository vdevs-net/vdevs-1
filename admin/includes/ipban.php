...
switch ($mod) {
    case 'new':
        ...
        */
        ...
        if (isset($_POST['submit'])) {
            ...
            $ban_url = isset($_POST['url']) ? functions::checkin($_POST['url']) : '';
            ...
            if (!$error) {
                ...
                switch ($ban_term) {
                    case 2:
                        echo $lng['redirect'] . '</p><p><h3>' . $lng['redirect_url'] . ':</h3>&nbsp;' . (empty($ban_url) ? $lng['default'] : htmlspecialchars($ban_url));
                        break;
                ...
                echo '</p><p><h3>' . $lng['reason'] . ':</h3>&nbsp;' . (empty($reason) ? $lng['not_specified'] : htmlspecialchars($reason)) . '</p>' .
                    '<input type="hidden" value="' . $ip1 . '" name="ip1" />' .
                    '<input type="hidden" value="' . $ip2 . '" name="ip2" />' .
                    '<input type="hidden" value="' . $ban_term . '" name="term" />' .
                    '<input type="hidden" value="' . htmlspecialchars($ban_url) . '" name="url" />' .
                    '<input type="hidden" value="' . htmlspecialchars($reason) . '" name="reason" />' .
    ...
    case 'insert':
        ...
        -----------------------------------------------------------------
        */
        $ip1 = isset($_POST['ip1']) ? abs(intval($_POST['ip1'])) : '';
        $ip2 = isset($_POST['ip2']) ? abs(intval($_POST['ip2'])) : '';
        $ban_term = isset($_POST['term']) ? abs(intval($_POST['term'])) : 1;
        $ban_url = isset($_POST['url']) ? functions::checkin($_POST['url']) : '';
        $reason = isset($_POST['reason']) ? functions::checkin($_POST['reason']) : '';
        if (!$ip1 || !$ip2) {
            echo functions::display_error($lng['error_address'], '<a href="index.php?act=ipban&amp;mod=new">' . $lng['back'] . '</a>');
        ...
        `ip2` = "' . $ip2 . '",
        `ban_type` = "' . $ban_term . '",
        `link` = "' . mysql_real_escape_string($ban_url) . '",
        `who` = "' . $login . '",
    ...
    case 'detail':
        ...
        */
        ...
        if (!mysql_num_rows($req)) {
            ...
            if ($res['ban_type'] == 2)
                echo '<br />&nbsp;' . htmlspecialchars($res['link']);
            echo '</p><p><h3>' . $lng['reason'] . '</h3>&nbsp;' . (empty($res['reason']) ? $lng['not_specified'] : htmlspecialchars($res['reason'])) . '</p></div>';
    ...
    default:
        ...
        */
        ...
        echo '<div class="phdr">' . $lng['total'] . ': ' . $total . '</div>';
        if ($total > $kmess) {
            echo '<div class="topmenu">' . functions::display_pagination('index.php?act=ipban&page=', $start, $total, $kmess) . '</div>';
